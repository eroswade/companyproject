...
{
    return (int)sqrt((double)vSize2(p0));
}
...
public:
    ...
    AABB()
    : pmin(LLONG_MIN, LLONG_MIN), pmax(LLONG_MIN, LLONG_MIN)
    {
    }
    AABB(Polygons polys)
    : pmin(LLONG_MIN, LLONG_MIN), pmax(LLONG_MIN, LLONG_MIN)
    ...
    {
        pmin = Point(LLONG_MAX, LLONG_MAX);
        pmax = Point(LLONG_MIN, LLONG_MIN);
        for(unsigned int i=0; i<polys.size(); i++)
        {
            for(unsigned int j=0; j<polys[i].size(); j++)
            {
                if (pmin.X > polys[i][j].X) pmin.X = polys[i][j].X;
                if (pmin.Y > polys[i][j].Y) pmin.Y = polys[i][j].Y;
                if (pmax.X < polys[i][j].X) pmax.X = polys[i][j].X;
                if (pmax.Y < polys[i][j].Y) pmax.Y = polys[i][j].Y;
            }
        }
    }
